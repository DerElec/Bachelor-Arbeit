...
variance_over_V = [get_real_value(variance[0])+get_real_value(variance[1])+get_real_value(variance[2]) for variance in variances]  # Assuming first variance as example
...
variance_over_Omega = [get_real_value(variance[0])+get_real_value(variance[1])+get_real_value(variance[2]) for variance in variances]  # Assuming first variance as example
...
def plot_heatmap_delta2(data, title, xlabel='Omega', ylabel='Delta_2', cmap='viridis'):
    plt.figure(figsize=(10, 8))
    ax = sns.heatmap(data, cmap=cmap, cbar_kws={'label': title})
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)
